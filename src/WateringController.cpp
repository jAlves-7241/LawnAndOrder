...
void WateringController::stop() {
    if (_active) {
        ...
        if (!_isWaiting && !_isRelayDeadTimeWaiting) {
            ...
            LOG_I("REGA", "Zona %d (%s) desactivada (interrompida)", _zoneIdx + 1, (_zoneIdx < NUM_ZONES) ? gState.zones[_zoneIdx].name : "N/A");
        }
        _active = false;
        _isWaiting = false;
        _isRelayDeadTimeWaiting = false;
...
void WateringController::update() {
    ...
    if (elapsed >= _zoneDurationMs) {
        ...
        _deactivateAll();
        LOG_I("REGA", "Zona %d (%s) desactivada", _zoneIdx + 1, (_zoneIdx < NUM_ZONES) ? gState.zones[_zoneIdx].name : "N/A");
        _queuePos++;
