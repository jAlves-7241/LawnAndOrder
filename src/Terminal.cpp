...
void Terminal::update() {
    // 1. Salvaguarda crítica: se a exportação de histórico estiver ativa, silenciar o terminal
...
void Terminal::_processCommand(char* cmd) {
    ...
    if (strcmp(trimmed, "help") == 0 || strcmp(trimmed, "?") == 0) {
        ...
        Serial.printf("Comando desconhecido: '%s'. Digite 'help' ou '?' para ver a lista de comandos.\n", trimmed);
    }
}

void Terminal::_cmdHelp() {
    Serial.println("======================================================================");
    Serial.println("                    LAWN & ORDER - SERVICO TERMINAL                  ");
    Serial.println("======================================================================");
    Serial.println("Comandos Disponiveis:");
    Serial.println("  help, ?                    - Mostra este menu de ajuda");
    Serial.println("  status                     - Relatorio em tempo real do sistema");
    Serial.println("  set_time AAAA-MM-DD HH:MM:SS - Define a data e hora local do relogio");
...
void Terminal::_cmdStatus() {
    ...
    if (gState.rtc_valid) {
        ...
        Serial.printf("  Hora Local:  %04d-%02d-%02d %02d:%02d:%02d (SOFTWARE ONLY)\n",
                      gState.now.year, gState.now.month, gState.now.day,
                      gState.now.hour, gState.now.min, gState.now.sec);
    ...
    if (gState.suspended) {
        uint32_t remaining = (gState.suspended_until > gState.now.unix) ? (gState.suspended_until - gState.now.unix) : 0;
        uint32_t hours = remaining / 3600;
        uint32_t mins = (remaining % 3600) / 60;
        Serial.printf("  Estado Rega: SUSPENSA (Resta %dh %dm)\n", hours, mins);
    } else if (gState.mode == AppMode::DESATIVADO) {
        Serial.println("  Estado Rega: INATIVA (Desativada no painel)");
    } else {
        Serial.printf("  Estado Rega: ATIVA (Proxima rega agendada para %02d:%02d)\n",
...
void Terminal::_cmdSetTime(char* args) {
    if (!args || *args == '\0') {
        Serial.println("Erro: Comando set_time requer argumentos: set_time AAAA-MM-DD HH:MM:SS");
        return;
    }

    int yr, mo, dy, hh, mm, ss;
    if (sscanf(args, "%d-%d-%d %d:%d:%d", &yr, &mo, &dy, &hh, &mm, &ss) != 6) {
        Serial.println("Erro: Formato de data/hora incorreto. Use: set_time AAAA-MM-DD HH:MM:SS");
        return;
    }
    ...
    if (dy < 1 || dy > maxDays) {
        Serial.printf("Erro: Dia invalido para o mes e ano especificados (%d/%d/%04d). O limite e %d dias.\n", dy, mo, yr, maxDays);
        return;
    }
    if (hh < 0 || hh > 23) {
    ...
    rtclock.set((uint16_t)yr, (uint8_t)mo, (uint8_t)dy, (uint8_t)hh, (uint8_t)mm, (uint8_t)ss);
    
    // Atualizar o scheduler imediatamente baseado no novo relógio
    scheduler.onModeChanged();

    Serial.printf("Hora definida com sucesso para: %04d-%02d-%02d %02d:%02d:%02d\n",
                  yr, mo, dy, hh, mm, ss);
}

void Terminal::_cmdExportConfig() {
    char hexStr[sizeof(AppConfigBlob) * 2 + 1];
    storage.exportConfigHex(hexStr);
...
void Terminal::_cmdImportConfig(char* hexStr) {
    if (!hexStr || *hexStr == '\0') {
        Serial.println("Erro: Comando import_config requer a string hexadecimal correspondente.");
        return;
    }
    ...
    if (storage.importConfigHex(hexStr)) {
        ...
        scheduler.onModeChanged();
        // Reposicionar ecrã físico para Idle para refletir as novas configurações imediatamente
        ui.goIdle();
